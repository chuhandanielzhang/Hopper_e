...
class HopperSimulation:
    ...
    def __init__(self, model_path=None, mode='leg', model=None, data=None, verbose=True):
        """
        初始化仿真
        ...
        if self.mode == 'prop':
            ...
            # 腿控 + 旋翼姿态辅助模式：使用完整三旋翼混控（参考 Quadrotor_SE3_Control-main）
    ...
    def compute_leg_control(self, state, desired_vel):
        ...
        """
        if self.use_estimator:
            est_vel, _ = self.com_filter.process(
                imu_accel=state['imu_acc'],
                imu_gyro=state['body_ang_vel'],
                imu_quat=state['body_quat'],
                foot_pos=state['foot_pos'],
                foot_vel=state['foot_vel'],
                state=self.controller.state
            )
            state['body_vel'] = est_vel
    ...
    def step_leg_with_prop_assist(self, desired_vel):
        ...
        # ========== 两个 Phase 都做姿态控制（强制水平） ==========
        # 目标姿态：roll=0, pitch=0
        target_roll = 0.0
        target_pitch = 0.0
        
        # 姿态误差
        e_roll = target_roll - rpy[0]
        e_pitch = target_pitch - rpy[1]
        ...
        if current_phase == 2:
            # ===== Stance Phase: 关闭旋翼 =====
            thrusts = np.zeros(3)
            motor_speeds = np.zeros(3)
            reaction_torques = np.zeros(3)
            Mx, My, Mz = 0.0, 0.0, 0.0
        else:
            # ===== Flight Phase: 开启旋翼姿态控制 =====
            total_thrust = base_thrust
            
            # PD 控制计算力矩
            Mx = self.Kp_roll * e_roll - self.Kd_roll * p
            My = self.Kp_pitch * e_pitch - self.Kd_pitch * q
            Mz = 0.0
            
            # 三旋翼动力分配
            motor_speeds = self.tri_rotor_mixer.calculate(total_thrust, Mx, My, Mz)
            ...
            # 限制总推力
            max_total_thrust = 0.2 * robot_mass * self.gravity
            actual_total = np.sum(thrusts)
            if actual_total > max_total_thrust:
