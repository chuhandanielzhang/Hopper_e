...
# 使用 Task1 优化后的参数（7.00s, 0.299m/s）
sim.controller.k = 1800     # 弹簧刚度（优化）
sim.controller.h = 0.30     # 目标跳跃高度
sim.controller.Kp = 5.0     # 能量补偿（优化）
sim.controller.Kv = 0.08    # 速度前馈（优化）
sim.controller.Kr = 0.012   # 速度校正（优化）
sim.controller.Kpp_x = 100  # 姿态比例
sim.controller.Kpp_y = 100
sim.controller.Kpd_x = 10.0 # 姿态微分
...
while sim.sim_time < total_time:
    ...
    cv2.putText(frame_bgr, text, (20, 300), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2)
    
    # 显示跳跃次数
    cv2.putText(frame_bgr, f'Bounces: {bounce_count}', (20, 340), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
