...
class RaibertController:
    ...
    def compute_torque(self, X, xdot, joint, jointVel, vel, quat, angVel, robotPos, desiredPos, rpy=None):
        ...
        if self.state == 1:
            ...
            # Raibert足端放置控制（与 Hopper4 2Dwork.py 第 270 行一致）
            # targetFootPos = Kv * v_current - Kr * v_desired
            # 注意：Hopper4.py 用 +，但 Hopper4 2Dwork.py 用 -
            # - 号更符合 Raibert 原理：
            #   - desiredVel > 0 (想向前) → tgt < 0 (腿在后方) → 加速向前
            #   - desiredVel < 0 (想向后) → tgt > 0 (腿在前方) → 减速
            # Kv: 当前速度反馈增益（预测飞行距离）
            # Kr: 期望速度增益（速度跟踪）
