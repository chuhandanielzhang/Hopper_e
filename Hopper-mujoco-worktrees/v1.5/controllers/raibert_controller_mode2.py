"""
Raibert + 虚拟弹簧控制器（MuJoCo 版本，Mode2）
...
class RaibertControllerMode2:
    ...
    def __init__(self):
        ...
        # ========== 相位切换阈值 ==========
        # MuJoCo 中需要更大的阈值来避免空中腿长波动导致的误切换
        # Hopper4.py 原始值: touchdown=0.02, liftoff=0.00
        # MuJoCo 调整值: 增大以避免空中波动
        self.flight_to_stance_threshold = 0.08  # 腿压缩阈值（增大：0.02→0.08）
        self.stance_to_flight_threshold = 0.02  # 腿伸展阈值（增大：0.00→0.02）
        self.Kpos = 0.0
        
        # ========== 相位切换去抖动 ==========
        # 最小保持时间（防止在阈值附近快速切换）
        self.min_phase_duration = 50  # 最少保持 50 个控制周期 (~50ms @ 1kHz)
    ...
    def compute_torque(self, X, xdot, joint, jointVel, vel, quat, angVel, robotPos, desiredPos, rpy=None, desired_rpy=None):
        """
        ...
        if self.state == 1:
            ...
            # Raibert 足端放置控制（与 Hopper4.py 完全一致）
        ...
        # ========== Stance Phase（与 Hopper4.py 第 362-451 行完全一致）==========
            ...
            if rpy is not None:
                # 统一期望姿态：默认 target=0；如提供 desired_rpy，则跟踪 desired_rpy
                if desired_rpy is None:
                    e_r = rpy[0]
                    e_p = rpy[1]
                else:
                    e_r = rpy[0] - desired_rpy[0]
                    e_p = rpy[1] - desired_rpy[1]
                hipTorque[0] = self.Kpp_x * e_r + self.Kpd_x * rAngVel[0]
                hipTorque[1] = self.Kpp_y * e_p + self.Kpd_y * rAngVel[1]
            else:
