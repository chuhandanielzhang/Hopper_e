...
class ModeELCMController:
    ...
    def __init__(self, *, modee_cfg: ModeEConfig | None = None, lcm_cfg: ModeELCMConfig | None = None):
        ...
        # alpha=0.3 means 30% new value + 70% old value (strong smoothing)
        self.pwm_filter_alpha = 0.3
    ...
    def _handle_imu_data(self, channel: str, data: bytes) -> None:
        ...
        with self.lock:
            ...
            # User report: gyro[1] (about +Y / pitch-rate) is sign-flipped vs our controller convention.
            # Convention used by ModeE: +X forward, +Y left, +Z up, so positive gyro_y should correspond
            # to positive pitch-rate about +Y. If the FC publishes the opposite, flip it here.
            gyro = np.array(msg.gyro, dtype=float).reshape(3)
            gyro[1] = -float(gyro[1])
            self.robot_state["imu_gyro"] = gyro
    ...
    def _publish_motor_pwm(self, pwm_us: np.ndarray, *, control_mode: int = 1) -> None:
        ...
        # Apply PWM low-pass filter (same as Hopper4 for propeller stability)
        # This smooths PWM changes and reduces high-frequency oscillations/wobble
        smoothed_pwm = self.pwm_filter_alpha * pwm_us + (1.0 - self.pwm_filter_alpha) * self.prev_pwm_us
        self.prev_pwm_us = smoothed_pwm.copy()
        pwm_us = smoothed_pwm
